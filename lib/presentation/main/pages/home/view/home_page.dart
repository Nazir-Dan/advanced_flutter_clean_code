...
class _HomePageState extends State<HomePage> {
  ...
  Widget _getContentWidget() {
    return StreamBuilder<HomeViewObject>(
      stream: _viewModel.outHomeViewObject,
      builder: (context, snapshot) {
        return Column(
  ...
  Widget _getBannerWidget(List<BannerAdd>? banners) {
    if (banners == null) {
      return Container();
    } else {
  ...
  _getServicesWidget(List<Service>? services) {
    if (services == null) {
      return Container();
    } else {
      return Padding(
  ...
  _getStoresWidget(List<Store>? stores) {
    if (stores == null) {
      return Container();
    } else {
      return Padding(
          ...
          children: [
            GridView.count(
              ...
              children: List.generate(stores.length, (index) {
                return InkWell(
                  onTap: () {
                    //navigate to store detailes screen
                    Navigator.pushNamed(context, Routes.storeDetailsRout);
                  },
                  child: Card(
                    elevation: AppSize.s4,
                    child: Image.network(
